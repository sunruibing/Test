$(document).ready(function(){
	...
	/* 获取医生信息*/
	$.ajax({
		url: 'http://139.224.196.16:80/worm/findDoctor',  
        type: 'post',  
        dataType: 'json',
	...
	//获取挂号列表
	$.ajax({
		url: 'http://139.224.196.16:80/Test/FindReminUser',  
        type: 'post',  
        dataType: 'json',
        success:function(data){
        	  $.each(data, function(commentIndex, comment){
        		  $("#remindTbody").html($("#remindTbody").html()+ "<tr id='theader'>"+
        					...
        					"<td >"+ '<span val=' + comment['id'] + '><img src="assets/images/doctor/03.png"/></span>'+"</td>"+
        					"</tr>");
	...
	$(".guahaotijiao").click(function(){
		$.ajax({
			url:'http://139.224.196.16:80/Test/AddRegUser',
			type:'post',
			dataType:'json',
